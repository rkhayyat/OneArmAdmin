...
export class FirebaseService {
  ...
  getArticleList(id:string,keyword:string): Observable<any> {
    return new Observable((observer: any) => {
      let path = "Articles";
        let onValueEvent = (snapshot: any) => {
          this.ngZone.run(() => {
            
            let results = this.handleSnapshot2(snapshot.value, id, keyword);
            // console.log(JSON.stringify(results))
             observer.next(results);
          });
        };
  ...
  handleSnapshot2(data: any, catId:string, keyword:string) {
    //empty array, then refill and filter
    this._allItems = [];
    if (data) {
      for (let id in data) {        
        let result = (<any>Object).assign({id: id}, data[id]);
         ...
         if ((String(result.text).search(keyword) > 0) || (keyword==='')) {
        //if(BackendService.token === result.UID){
          if (catId) {
            if(result.categoryId === catId){
               this._allItems.push(result);
